...
STATUS_INFERENCE_CURL_CONNECT_TIMEOUT="${CLAWBOX_STATUS_INFERENCE_CURL_CONNECT_TIMEOUT:-${CLAWBOX_STATUS_CURL_CONNECT_TIMEOUT:-5}}"
...
vm_llama_inference_probe() {
  ...
  status_debug "VM inference connect timeout=$STATUS_INFERENCE_CURL_CONNECT_TIMEOUT"
  status_debug "VM inference max time=$STATUS_INFERENCE_CURL_MAX_TIME"

  vm_ssh_exec sh -s -- "$VM_LLAMA_COMPLETION_URL" "$STATUS_INFERENCE_CURL_CONNECT_TIMEOUT" "$STATUS_INFERENCE_CURL_MAX_TIME" "$STATUS_DEBUG" <<'EOF'
url="$1"
connect_timeout="$2"
max_time="$3"
debug="$4"
