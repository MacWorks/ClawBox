...
- On fresh setup, after an existing UTM VM is selected, ClawBox installs the same `com.clawbox.startutmvm` user LaunchAgent used for login auto-start, bootstraps it, and relies on the plist `RunAtLoad` behavior rather than issuing a redundant setup-time kickstart. At this early stage the wrapper may receive only the selected VM name; it treats empty `VM_HOST` as start-only mode and verifies UTM runtime state instead of SSH reachability before setup proceeds to VM address prompts.
- The setup-time LaunchAgent is temporary infrastructure unless the later Runtime step asks and the user chooses to keep VM auto-start at login. Declining that prompt unloads and removes the service before setup completes.
