...
- On fresh setup, after an existing UTM VM is selected, ClawBox installs and kickstarts the same `com.clawbox.startutmvm` user LaunchAgent used for login auto-start. At this early stage the wrapper may receive only the selected VM name; it treats empty `VM_HOST` as start-only mode and verifies UTM runtime state instead of SSH reachability before setup proceeds to VM address prompts.
